"""G1 -- no test module sits outside the guarantee registry.
...
Two modules in a sibling repository were in exactly that position -- its
contract test and its fixture-axis test, 16 tests carrying the anti-prose
controls and the controls on the fixtures its DST guarantee was measured with.
A review skipped one at module level and the suite reported 124 passed, exit
0, with nothing in CI referencing it. Another sibling shipped the same shape:
...
def test_a_module_that_PLANTS_and_registers_nothing_cannot_be_excused():
    ...
    body = (
        ...
        "    with planted('__init__.py', 'Open Parking AI', 'Open Parking AI'):\n"
        "        pass\n"
    )
    try:
        intruder.write_text(body)
