...
| **G6** | An expired token and an already-used token are each refused BY NAME, distinctly from an unknown one and from a cancelled one. Expiry is derived from the stated expires_at against the instant given and is never typed; a token superseded by a newer one is cancelled with that reason; a spent token spends once, and the spend asserts one row. |
