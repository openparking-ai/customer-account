...
| `REFUSAL_KDF_UNKNOWN` | A credential row found carrying a KDF this module does not have -- anything but scrypt -- is refused by this name when a password is checked against it: through verify-password, and through an email change authorised by the current password. The schema's CHECK refuses the write, and an owner can alter a schema; a row outside it was written past the schema, and the module says so rather than guessing how the hash was made. The password is never compared, so this is a refusal and not WRONG_PASSWORD. Nothing is written. |
...
That is 30 refusals, every one raised somewhere in the package and none raised anywhere that is not here (a test reads the raise sites from the AST).
<!-- END:refusals -->
